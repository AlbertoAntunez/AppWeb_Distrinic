...
BEGIN TRANSACTION 
	BEGIN
		...
		VALUES (@CuentaMP,1,@Mes_operativo,@NroAsiento,@pFecha,@pObs,@Tc,SUBSTRING(@IdComprobante,1,4),SUBSTRING(@IdComprobante,5,8),SUBSTRING(@IdComprobante,12,1),'D',@pImporte,'   1',1,@Periodo,@pImporte,
		...
		NUMERO,LETRA,[DEBE-HABER],IMPORTE,MONEDA,COTIZACION,PERIODO,CABIMPORTE,TIPO_REG,CONTABILIZADO,
		FECHAHORA_GRABACION,FechaSubdiario, USUARIO_LOGEADO, IDCAJAS)
		VALUES (@pCliente,2,@Mes_operativo,@NroAsiento,@pFecha,@pObs,@Tc,SUBSTRING(@IdComprobante,1,4),SUBSTRING(@IdComprobante,5,8),SUBSTRING(@IdComprobante,12,1),'H',@pImporte,'   1',1,@Periodo,@pImporte,
		@Tc,0,@pFecha,@FechaHoraGrabacion,@USUARIO,'   1')
